...
from .serializers import SwapSerializer, PlatformMessageSerializer, BannedUserSerializer, UserActivityLogSerializer
...
@api_view(['GET'])
@permission_classes([IsAdminUser])  # use @admin_required decorator instead in prod
def download_report(request):
    try:
        # Step 1: Fetch data
        users = User.objects.all().values('id', 'email', 'name', 'location', 'availability', 'is_public')
        swaps = Swap.objects.all().values('id', 'requester__email', 'receiver__email', 'status', 'created_at')
        logs = UserActivityLog.objects.all().values('id', 'user__email', 'action', 'timestamp', 'details')
        ...
        # Also for datetime without timezone to datetime objects (just in case)
        for df in [df_swaps, df_logs]:
            for col in df.columns:
                if pd.api.types.is_datetime64_any_dtype(df[col]):
                    df[col] = pd.to_datetime(df[col]).dt.tz_localize(None)

        # Step 3: Create Excel in memory
        output = BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            # --- Pie Chart: Users by availability ---
            if not df_users.empty and 'availability' in df_users.columns:
                ...
                avail_counts.to_excel(writer, sheet_name='Charts', startrow=1, index=False)
            ...
            if not df_logs.empty and 'timestamp' in df_logs.columns:
                ...
                daily_logs.to_excel(writer, sheet_name='Charts', startrow=20, index=False)
